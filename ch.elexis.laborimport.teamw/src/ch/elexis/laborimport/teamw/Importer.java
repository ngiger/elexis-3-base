...
public class Importer extends ImporterPage {
	...
	private Result<?> importDirectFtp(){
		Result<String> result =
			new Result<>(ch.elexis.laborimport.teamw.Messages.Importer_ok); //$NON-NLS-1$
		...
		try {
			List<String> hl7FileList = new Vector<>();
			try {
			...
			result = new Result<>(SEVERITY.ERROR, 1, e.getMessage(), MY_LAB, true);
			...
			result = new Result<>(SEVERITY.WARNING, 1, e.getMessage(), MY_LAB, true);
			ResultAdapter.displayResult(result,
				ch.elexis.laborimport.teamw.Messages.Importer_error_import); //$NON-NLS-1$
		}
	...
	private Result<?> importDirectBatch(){
		...
		if (batchFile == null || batchFile.length() == 0) {
			return new Result<>(SEVERITY.ERROR, 1,
	...
	private class LabImporter extends Composite {
		...
		public LabImporter(final Composite parent, final ImporterPage home){
			...
			bFile.setText(ch.elexis.laborimport.teamw.Messages.Importer_label_importFile);
			...
			String direktHerkunft =
				ch.elexis.laborimport.teamw.Messages.Importer_ftp_label;
			if (PreferencePage.BATCH.equals(batchOrFtp)) {
				direktHerkunft =
					ch.elexis.laborimport.teamw.Messages.Importer_batch_label;
			}
			bDirect = new Button(this, SWT.RADIO);
			bDirect.setText(ch.elexis.laborimport.teamw.Messages.Importer_label_importDirect +
				" (" + direktHerkunft + ")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
